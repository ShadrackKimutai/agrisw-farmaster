...
Public Class impl
    ...
    Private Sub chemstorage()
        ...
        strg = "Select * from STORAGEHQ WHERE CATEGORY LIKE '%CHEMICAL%' AND QTTY >2"
        Try
            cnn1.Open()
    ...
    Private Sub chemREQS()
        ...
        connectionstring = "Provider=SQLOLEDB;Data Source=(local);Persist Security Info=True;Password=swiftshady;User ID=sa;Initial Catalog=NemesisFarmaster"
        cnn1 = New OleDb.OleDbConnection(connectionstring)
        strg = "Select Department,OrderNo,category,Item_Type,Item_Name,Quantity,Units,DOReq,Urgency,RO from REQUESTHQ WHERE CATEGORY LIKE '%CHEMICAL%' and status =1"
        Try
            cnn1.Open()
            adpt = New OleDb.OleDbDataAdapter(strg, cnn1)
    ...
    Public Sub listviewfill()
        ...
        Try
            ...
            Dim command As New SqlCommand("SELECT   Department,OrderNo,category,Item_Type,Item_Name,Quantity,Units,DOReq,status,RO from requesthq", conn)
